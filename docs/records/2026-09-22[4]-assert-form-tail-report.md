...
> **Состояние на 2026-09-22:** сделано и смержено, `77ea78a`. Один вывод
> в тексте ниже — «голый ассерт недостижим» — позже опровергнут замером,
> см. поправку в самом разделе.
...
> не ассерт.
> **Связанные записи:**
> `2026-09-22[5]-held-over-dependency-report.md`.
> **Связанные записи:**
...
> **Поправка 2026-09-22.** Замер выше неполон, и вывод из него неверен. Обе
> пробы ставят лист **под группу**, где фильтр действительно не пускает до
> ассерта. Лист, стоящий **корнем**, группы над собой не имеет: контейнер
> зовёт `root.init()` напрямую, и ассерт срабатывает — голым сообщением, без
> единого слова. То есть он был достижим всё это время. Отказ ему написан
> тогда же, когда и отказ пропущенному под группой:
> `docs/records/2026-09-22[5]-held-over-dependency-report.md`.
