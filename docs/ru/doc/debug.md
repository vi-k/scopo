...
> Перевод `doc/debug.md` (blob `e11f017195f70062edd4519f4a8de6df16a1781d`).
...
проходят её `init()` и `dispose()`. Четыре шаговых события выше — это её шаги,
...
- **зависимость вашего собственного изготовления входа не объявляет.** Той,
  ...
  по-прежнему приходят: `onProgress` едет потоком `init()`, то есть ровно той
  частью договора, которую такая зависимость и реализует, а её освобождение
  объявляют за неё — группа над ней, когда путь проходит через неё, или, если
  такая зависимость и есть всё дерево, слушатель самого контейнера. Последний
  случай — единственное место, где обещание ниже кончается: дерево, чей корень
  ваш и чей разбор ведёт кто-то другой, не объявляется вовсе, потому что поток
  читает не контейнер, а читать ему больше нечего.
...
того, чтобы начинать второй; в обоих случаях поток читает не он.
`onDisposalProgress` отправляют, когда диспозер вернулся, а не когда путь дошёл
до слушателя, поэтому пара переживает все три случая. Переживает она и обход,
отменённый на запаркованном диспозере: освобождение, которое всё-таки
состоялось, объявляют.
...
скоупа, и до контейнера потому, что едет потоком инициализации контейнера, а
скоуп передаёт дальше всё, что этот поток выдал. `onStepStarted` и
